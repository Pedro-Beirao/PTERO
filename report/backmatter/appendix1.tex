\chapter{Further Details of the Implementation} \label{app:implementation}
This appendix contains implementation details that were not important enough to include in \Cref{chap:implementation}. Most of the content here are big listings that would break the flow of the chapter, so this place was devised for displaying them.

\section{Function Block Files}
Examples of the formats of the \textit{.fbt} and \textit{.py} files that make up a function block are represented in \Cref{lst:fb-xml} and \Cref{lst:fb-py}, respectively.
...
<FBType Name="NEW_FB_1">
  <InterfaceList>
    ...
    <InputVars>
      ...
      <VarDeclaration Name="VARIABLE_2" Type="INT"/>
    </InputVars>
    <OutputVars>
...
\Cref{lst:sync-config} presents an example configuration that the \texttt{synchronize.py} script requires in order to send the files to the \textit{DINASORE} devices.
...
The previously mentioned methods to build the messages are replicated in \Cref{lst:build-message-py,lst:build-message-js}.
...
Next, depending on which nodes are mapped to each machine, it needs to \texttt{CREATE} all the mapped nodes and \texttt{WRITE} the initial input values with the commands in \Cref{lst:deploy-create-nodes}. The configuration name in these and all following commands will be "\textit{EMB\_RES}" as they need to be bound to it in order to be initialized and ran.
...
After creating the watches, request can be sent in order to receive the watch values using the command in \Cref{lst:deploy-read-watch}. This will return the response in \Cref{lst:deploy-watch}, which by parsing the values, allows the prototype to display them in the \textit{Graph View}.
