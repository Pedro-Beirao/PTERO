...
An alternative is \textit{FBME} which is a much simpler modelling environment based on the \textit{JetBrains MPS} language workbench. The main contribution of this work is the research on applying model driven development principles on the architecture of the \ac{IDE} itself \cite{sorokin_novel_2022}.
...
\resizebox{\textwidth}{!}{%
\begin{tabular}{@{}lllcp{5cm}@{}}
\toprule
\textbf{Tool} & \textbf{Type} & \textbf{Deployment} & \textbf{\shortstack{Real-time\\collaboration}} & \textbf{Comments} \\
\midrule
4diac IDE \cite{zoitl_developing_2013}
  & Open source & Desktop
  & \cno
  & Steep learning curve; heavyweight Eclipse base \\[6pt]
FBME \cite{sorokin_novel_2022}
  & Open source & Desktop
  & \cno
  & Simpler UX; relies on JetBrains MPS \\[6pt]
EcoStruxure AE
  & Commercial & Desktop
  & \cno
  & Vendor lock-in; per-seat licensing \\[6pt]
ISaGRAF Workbench \cite{vyatkin_comparisons_2008}
  & Commercial & Desktop
  & \cno
  & Vendor lock-in; licensing costs \\[6pt]
Rohat et al.\ \cite{rohat_remote_2014}
  & Proposal & \shortstack[l]{Web\\(monitoring only)}
  & \cno
  & Read-only; no authoring; dated web tech \\[6pt]
Lyu et al.\ \cite{lyu_towards_2024}
  & Proposal & \shortstack[l]{Web\\(embedded desktop IDE)}
  & \cno
  & Does not take advantage of web technologies for the modelling system \\
PTERO\
  & New Proposal & Web
  & \cyes
  & Does not take advantage of web technologies for the modelling system \\
